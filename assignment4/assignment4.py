...
class IRSystem:
    ...
    def boolean_retrieve(self, query):
        ...
        while (query and docs):
            query_list = self.inv_index[query.pop()]
            docs = self.list_intersect(docs, query_list)
    ...
    def compute_tfidf(self):
        ...
        for i in range(num_docs):
            doc_word_freq = Counter(self.docs[i])
            doc_word_set = set(self.docs[i])
            for word in doc_word_set:
                #print(doc_word_freq[word])
                tf = 1 + math.log(doc_word_freq[word], 10)
                idf = math.log(num_docs / len(self.inv_index[word]), 10)
                tfidf[(word, i)] = tf * idf
    ...
    def rank_retrieve(self, query):
        ...
        words_in_query = Counter(query)
        log_tf_query = defaultdict(lambda:0)

        for word in words_in_query:
            log_tf_query[word] = 1 + math.log(words_in_query[word], 10)
        ...
        for i in range(len(self.docs)):
            ...
            doc_word_set = set(self.docs[i])
            for word in doc_word_set:
                doc_tf_idf = self.tfidf[(word, i)]
                upper_value += log_tf_query[word] * doc_tf_idf
                lower_value += doc_tf_idf * doc_tf_idf
                
            scores[i] = upper_value / math.sqrt(lower_value)
